...
func newTestDB(t *testing.T) *sql.DB {
	...
	// in one db.Exec() call.
	db, err := sql.Open("mysql", "snippetbox:WN85VCBUXGPRv2hkuHwmQtySFz4Y7b6aJZ3jLAfqsKp9eMnDrE@tcp(localhost:6033)/test_snippetbox?parseTime=true&multiStatements=true")
	if err != nil {
		t.Fatal(err)
	}
